...
{
  cg::renderer::renderer* renderer =
    reinterpret_cast<cg::renderer::renderer*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
  ...
  case WM_MOUSEMOVE: {
    if (renderer)
    {
      ...
      // renderer->move_yaw((2.f*static_cast<float>(x_pos) /
      // renderer->get_width()-1.f)*30.f); renderer->move_pitch((-2.f*static_cast<float>(y_pos)
      // / renderer->get_height()+1.f)*30.f);
    }
  }
    return 0;
