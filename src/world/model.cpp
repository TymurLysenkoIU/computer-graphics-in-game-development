...
void cg::world::model::load_obj(const std::filesystem::path& model_path)
{
	...
	vertex_buffer = std::make_shared<cg::resource<cg::vertex>
	>(vertex_buffer_id);

	per_shape_buffer.resize(shapes.size());
	for (size_t s = 0; s < shapes.size(); ++s)
	{
		per_shape_buffer[s] = std::make_shared<cg::resource<cg::vertex>>(
			per_shapes_ids[s]);
	}
...
std::shared_ptr<cg::resource<cg::vertex>>
	cg::world::model::get_vertex_buffer() const
{
